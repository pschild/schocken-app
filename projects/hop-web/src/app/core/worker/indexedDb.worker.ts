...
addEventListener('message', async (event: MessageEvent) => {
  ...
  if (workerMessage.action === 'countEventTypeById' && workerMessage.payload.eventTypeId) {
    try {
      const roundEventsCount = await countEventTypeById(workerMessage.payload.eventTypeId);
